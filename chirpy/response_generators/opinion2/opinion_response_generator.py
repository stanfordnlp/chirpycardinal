...
from chirpy.response_generators.opinion2.opinion_regex_templates import WhatsYourOpinion
...
from chirpy.response_generators.opinion2.constants import ACTION_SPACE, solicit_reason
...
from chirpy.core.util import DAYS_OF_WEEK, catch_errors, contains_phrase, get_eastern_dayofweek
...
class OpinionResponseGenerator2(ResponseGenerator):
    ...
    def select_phrase_for_prompt(self, state : State) -> Tuple[Optional[str], PromptType, str]:
        ...
        if hasattr(self.state_manager.current_state, 'entity_tracker'):
            cur_entity = self.state_manager.current_state.entity_tracker.cur_entity # type:ignore
                ...
                for ent_group_name, ent_group in ENTITY_GROUPS_FOR_CLASSIFICATION.ordered_items:
                    if ent_group.matches(cur_entity):
                        ...
                        if len(phrases_of_ent_group) > 0:
                            ...
                            return phrase, priority, f"On the subject of {ent_group_name}, "
        ...
        if len(recent_available_phrases) > 0:
            ...
            return phrase, priority, self.state_manager.current_state.choose_least_repetitive(MENTION_REMEMBER_TRANSITIONS) # type: ignore
        ...
        if len(generic_available_phrases) > 0:
            ...
            return phrase, priority, self.state_manager.current_state.choose_least_repetitive(DO_YOU_LIKE_TRANSITIONS) # type: ignore
    ...
    def update_state_if_chosen(self, state: State, conditional_state : Optional[State]) -> State:
        ...
        return conditional_state or state
