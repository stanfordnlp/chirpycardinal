...
from chirpy.core.regex.templates import DontKnowTemplate, EverythingTemplate, NotThingTemplate, RESPONSE_TO_DONT_KNOW, \
    RESPONSE_TO_BACK_CHANNELING, RESPONSE_TO_EVERYTHING_ANS, RESPONSE_TO_NOTHING_ANS, BackChannelingTemplate
from chirpy.response_generators.neural_helpers import get_random_fallback_neural_response, neural_response_filtering
from chirpy.response_generators.wiki.treelets.abstract_treelet import Treelet, HANDOVER_TEXTS, \
    CONVPARA_BLACKLISTED_ENTITIES
...
from chirpy.response_generators.wiki.treelets.convpara_til_treelet import did_you_know
from chirpy.response_generators.wiki.treelets.handle_section_treelet import APOLOGY_TEXTS
from chirpy.response_generators.wiki.treelets.open_questions import OPEN_QUESTIONS_DICTIONARY
...
def open_answer_prompts(entity: WikiEntity, repeat=False):
    prompts = [
        #f"What {'else ' if repeat else ''}do you find interesting about {entity.common_name}?",
        #f"{entity.common_name}! So interesting. Do you find them interesting too?",
        #f"Talking about {entity.common_name}. So unique, what do you think?",
        ...
        #         f"You mentioned {entity.common_name}. What {'other ' if repeat else ''}parts of {entity.common_name} do you find interesting?",
        #f"What {'else ' if repeat else ''}makes you interested in {entity.common_name}?"
     ]
    return prompts
...
class OpenQuestionTreelet(Treelet):
    ...
    def __repr__(self):
        return "Open Question Treelet (WIKI)"
    ...
    def search_highlights(self, state) -> List[WikiSection]:
        ...
        if not (noun_phrases or linked_spans):
            search_sections = wiki_utils.search_wiki_sections(entity.name, tuple([utterance]), tuple(linked_wikilinks))
        else:
            search_sections = wiki_utils.search_wiki_sections(entity.name, tuple(noun_phrases+linked_spans), tuple(linked_wikilinks))
        logger.debug(
    ...
    def get_prompt_question(self, state, new_entity: Optional[WikiEntity] = None) -> Tuple[str, ConditionalState]:
        ...
        if len(state.entity_state[cur_entity.name].highlights_used) >= 2:
            raise CantPromptError("Already gave open response twice, not prompting anymore")
        if len(state.entity_state[cur_entity.name].neural_fallbacks_used) >= 2:
            raise CantPromptError("Already gave neural fallbacks twice, not prompting anymore")

        for ent_group_name, ent_group in ENTITY_GROUPS_FOR_CLASSIFICATION.ordered_items:
            if ent_group.matches(cur_entity) and ent_group_name in OPEN_QUESTIONS_DICTIONARY and OPEN_QUESTIONS_DICTIONARY[
                ent_group_name]:
                break
        else:
            ent_group_name = None
        if ent_group_name:
            ...
            if len(possible_open_question_types - entity_open_question_types_asked) ==0:
                ...
                if len(never_asked_possible_open_question_types) == 0:
                    ...
                    open_question = self.rg.state_manager.current_state.choose_least_repetitive(list(questions2rettype.keys()))
                    ...
                    conditional_state = ConditionalState(cur_doc_title=cur_entity.name, prompt_handler=self.__repr__(),
                ...
                # Ask a question type which has never been asked
                    ...
                    for return_type, questions in OPEN_QUESTIONS_DICTIONARY[ent_group_name].items():
                        if return_type in never_asked_possible_open_question_types and questions:
                            open_question = self.rg.state_manager.current_state.choose_least_repetitive(questions)
                            logger.primary_info(
                                f'WIKI has an open question {open_question} for {cur_entity} matching EntityGroup: "{ent_group_name}"')
                            text = open_question.format(entity=cur_entity.common_name)
                            conditional_state = ConditionalState(cur_doc_title=cur_entity.name, prompt_handler=self.__repr__(),
            ...
            if len(state.entity_state[cur_entity.name].highlights_used) >=1 or len(state.entity_state[cur_entity.name].neural_fallbacks_used) >= 1:
                raise CantPromptError("Already asked a generic open question once, not prompting anymore")
            ...
            text = self.rg.state_manager.current_state.choose_least_repetitive(
                open_answer_prompts(cur_entity, repeat=len(state.entity_state[cur_entity.name].highlights_used) > 0))
            conditional_state = ConditionalState(cur_doc_title=cur_entity.name, prompt_handler=self.__repr__(),
    ...
    def handle_prompt(self, state: State) -> ResponseGeneratorResult:
        ...
        if DontKnowTemplate().execute(utterance) is not None:
            handover_response = self.rg.state_manager.current_state.choose_least_repetitive(RESPONSE_TO_DONT_KNOW)
        elif BackChannelingTemplate().execute(utterance) is not None:
            handover_response = self.rg.state_manager.current_state.choose_least_repetitive(RESPONSE_TO_BACK_CHANNELING)
        #elif EverythingTemplate().execute(utterance) is not None:
        #    handover_response = self.rg.state_manager.current_state.choose_least_repetitive(RESPONSE_TO_EVERYTHING_ANS)
        elif NotThingTemplate().execute(utterance) is not None:
            handover_response = self.rg.state_manager.current_state.choose_least_repetitive(RESPONSE_TO_NOTHING_ANS)
        elif self.is_no(utterance):
            handover_response=self.rg.state_manager.current_state.choose_least_repetitive(HANDOVER_TEXTS)
        ...
        for selected_section in search_sections:
            ...
            section_snippet = wiki_utils.clean_wiki_text(selected_section.highlight)
            ...
            if convpara_snippet:
                ...
                if section_snippet[-1] not in ['.', '!', '?']:
                    section_snippet += '.'
                conditional_state = ConditionalState(cur_doc_title=entity.name, discussed_section=selected_section,
                                                     responding_treelet=self.__repr__(), highlight_used=(section_snippet, selected_section))
                base_response_result = ResponseGeneratorResult(text=section_snippet, priority=ResponsePriority.STRONG_CONTINUE,
                                                               needs_prompt=True, state=state, cur_entity=entity,
                                                               conditional_state=conditional_state)
            ...
            gpt2ed_module = GPT2ED(self.rg.state_manager)
            responses = gpt2ed_module.execute()
            ...
            if not responses:
                ...
                chosen_response = self.rg.state_manager.current_state.choose_least_repetitive(APOLOGY_TEXTS)
                state.entity_state[entity.name].finished_talking = True
                ...
                if not (chosen_response[-1] in ['.', '!']):
                    chosen_response += '.'
                logger.primary_info(f"Chose random neural fallback response {chosen_response} in Open question treelet")
                #needs_prompt=False
                #if len(state.entity_state[entity].neural_fallbacks_used) >= 1:
                #    logger.primary_info(
                #        f"This is the second time we had to choose a neural fallback response in Open question treelet, "
                #        f"so marking it as needing a prompt")
                #    needs_prompt=True
                response_result = ResponseGeneratorResult(text=chosen_response,
                                                           priority=ResponsePriority.STRONG_CONTINUE,
                                                           needs_prompt=True, state=state, cur_entity=entity,
                                                           conditional_state=ConditionalState(
                                                               neural_fallback=chosen_response,
                                                               responding_treelet=self.__repr__(),
                                                           ))
