...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        keep_layers = kwargs.pop("keep_layers", [])
        ...
        if from_tf:
            ...
            # DECODER SMASH!
            removed_keys = []
            BART_NUM_DECODERS = 12
            if keep_layers:
                assert len(keep_layers) > 0, "Must keep at least one decoder layer!"
                for key in state_dict.copy():
                    for i in range(BART_NUM_DECODERS):
                        if f'decoder.layers.{i}.' not in key: continue
                        if i in keep_layers:
                            new_i = keep_layers.index(i)
                            if new_i != i:
                                new_key = key.replace(f'decoder.layers.{i}.', f'decoder.layers.{new_i}.')
                                state_dict[new_key] = state_dict.pop(key)
                            else:
                                state_dict.pop(key)
                                removed_keys.append(key)

            ### ETHAN ADDITION TO SUPPORT SMASHING THE DECODER
            modify_decoder_layers = False
            if modify_decoder_layers:
                keep_layers = [0, 1, 2, 3,  4, 5]
                del_layers = [3]
                keys = [key for key in state_dict.keys()] # do a hacky copy :)
                for key in keys:
                    for i in range(6):
                        if f'layers.{i}' not in key: continue
                        if i in keep_layers:
                            new_layer = keep_layers.index(i)
                            if new_layer == i: continue
                            new_key = key.replace(f'layers.{i}', f'layers.{new_layer}')
                            print("New key is", new_key, key)
                            state_dict[new_key] = state_dict.pop(key)
                        else:
                            state_dict.pop(key)
